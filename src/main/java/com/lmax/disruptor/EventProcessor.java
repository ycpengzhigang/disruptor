 ...
 * 
 * EventProcessor——事件处理器，监听RingBuffer的事件，并消费可用事件，从RingBuffer读取的事件会交由实际的生产者实现类来消费；
 * 它会一直侦听下一个可用的序号，直到该序号对应的事件已经准备好。
 * 
 */
public interface EventProcessor extends Runnable
{
     ...
     * 发出信号，表明此EventProcessor在下一次清理中断完成消费时应该停止。
     * It will call {@link SequenceBarrier#alert()} to notify the thread to check status.
     * 它将调用{@link SequenceBarrier#alert()}通知线程检查状态。
     * 注:停止
     */
    void halt();
